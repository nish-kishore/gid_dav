...
map_25_e_noarrows <-
  ...
  #                  mid_place = 1, col = "grey50", size = 1, curvature = .3, arrow_fins = arrow_head_wings(),
...
map_25_n <-
  ...
  ggplot2::geom_sf(data = all,  fill = "#FFFFFF") +
  # ggplot2::geom_sf(data = prov2,  lwd = 0.1, fill = NA, show.legend = FALSE) +
  ggplot2::geom_sf(data = zas3_a ,fill = "red", color = "grey20", lwd = 0.4) +
  # geom_arrow_curve(data=zas3_a %>% filter(count>10000), aes(x = CENTER_LON, y = CENTER_LAT,
  #                                                           xend = US_CITY_LON, yend = US_CITY_LAT),
  #                  mid_place = 1, col = "black", size = 1, curvature = .3, arrow_head = arrow_head_wings(), arrow_fins = arrow_head_wings(),
  #                  lineend = "square") +
  ggplot2::theme_bw() +
  scale_fill_brewer(name= "Travelers\n to US:\n(2023)" , palette = "Reds") +
  theme(legend.position = "none",
...
map_25_l <-
  ...
  geom_arrow_curve(data=zas3_a %>% filter(count>10000), aes(x = CENTER_LON, y = CENTER_LAT, 
                                                            xend = US_CITY_LON, yend = US_CITY_LAT), 
                   mid_place = 1, col = "black", size = 1, curvature = .3, arrow_head = arrow_head_wings(), 
                   lineend = "square") + 
  ggplot2::theme_bw() +
  scale_fill_brewer(name= "Travelers\n to US:\n(2023)" , palette = "Reds") +
...
ggsave(plot = map_25_n, filename = "./output/zas_map_wings.jpeg", height = 2.5, width = 4, unit = "in", dpi = 300)
