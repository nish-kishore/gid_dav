...
required.packages <- c("tidyverse", "AzureStor","Microsoft365R", "readxl", "janitor","scales", "devtools", "rvest", "sf")
...
measles_deaths <- sirfunctions::edav_io(io = "read", default_dir = "GID/GIDMEA/giddatt",
                                       file_loc = "data_raw/Measlesdeaths_1981-2023.csv") %>% 
  mutate(vpd="Measles", source="Penn State Model (Internal)")
# year, cases, cases_lb, cases_ub, deaths, deaths_lower_bound, deaths_upper_ bound, dalys, cfr, cfr_lb, cfr_ub
# country looks like iso3
# add CI
# Variables: cases_modeled, deaths_modeled, dalys, cfr
...
## GAVI FUNDING DATA ###########################################
## Source: Gavi website (public)

## update after update to function pushed
# gavi_commitments <- sirfunctions::edav_io(io = "read", default_dir = "GID/GIDMEA/giddatt",
#                                         file_loc = "data_raw/2024-09-30_Gavi-Commitments-Approvals-Disbursements.xls",
#                                         sheet = "Database - ALL & HISTORICAL-PFM") %>% 
#   mutate(source="Gavi website")

## for now, load excel file using explore_edav interaction
#gavi_commitments <- explore_edav("GID/GIDMEA/giddatt/data_raw")

# pull only last sheet from excel file that contains all data in table
gavi_commitments <- gavi_commitments$`Database - ALL & HISTORICAL-PFM`
gavi_commitments <- gavi_commitments %>% mutate(source="Gavi website") %>% 
  rename(variable = Measures, value=Amount, year=`Budget Period`)

# unique(gavi_commitments$`Fund Type`) # Fund type is PEF or PFM
# unique(gavi_commitments$`Priority Description`) # Vaccine or Cash or NA
# IF NA - applicant description: TCA, NA, PTG, COVAX, or SFA (funding buckets)

## Variables of interest (all in $USD)
gavi_commitments$variable[variable=="ENDB & PREB"] <- "Gavi_funds_committed_usd" # Committed - positive signal to pay but not liable
gavi_commitments$variable[variable=="APRB & OPRB"] <- "Gavi_funds_approved_usd" # Approved - subset of commitments for which funding available & performance assessed
gavi_commitments$variable[variable=="Payments in USD"] <- "Gavi_funds_disbursed_usd" # Disbursed - subset of approved funds that were disbursed

### Extract countries
## match sequence of digits ([0-9]+) at the start of the string (^)
## followed by (+) one or more whitespace characters (\\s+)
gavi_commitments$country_name <- sub("^[0-9]+\\s+", "", gavi_commitments$`Funds Center`)

## match country ref data
# setdiff(gavi_commitments$country_name %>% unique(),
#         country_ref$country_name %>% unique())

## update values to match ref data
gavi_commitments <- gavi_commitments %>% mutate(
  country_name = case_when(
    ...
    country_name == "Marshall Islands (The)" ~ "Marshall Islands",
    country_name == "Korea DPR" ~ "Democratic People's Republic of Korea",
    country_name == "Lao PDR" ~ "Lao People's Democratic Republic",
    country_name == "Whole Syria" ~ "Syrian Arab Republic",
    country_name == "Occupied Palestinian Territory" ~ "occupied Palestinian territory, including east Jerusalem",
    T ~ country_name
  )
)

# CHECK - all country names match ref data (TRUE)
length(setdiff(gavi_commitments$country_name %>% unique(),
                 country_ref$country_name %>% unique())) ==0 # TRUE

# add iso3_code
gavi_commitments <- gavi_commitments %>% left_join(
  (country_ref %>% select(country_name, iso3_code)), by="country_name"
)

# Write wide data table to clean folder, may be accessed separately
sirfunctions::edav_io(io = "write", default_dir = "GID/GIDMEA/giddatt",
                      file_loc = "data_clean/gavi_commitments.rds", obj = gavi_commitments)

gavi_commitments

## DENGUE CASE ESTIMATES ###########################################
## CERVICAL CANCER CASE ESTIMATES (HPV) ###########################################
