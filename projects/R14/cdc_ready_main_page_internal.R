...
# For GID OD - Simmilar to EOC Maps pages / includes ISO codes for tables and lodas file to EDAV when complete 
...
# Base for table
t2_overall <- p1 %>%
  group_by(country.iso3, place.admin.0, m_sub2) %>%
  count() %>%
...
meta <- tibble() %>%
         add_row() %>%
         mutate(
           Global_Data_Update = raw.data$metadata$download_time, 
           Data_Update = today())
